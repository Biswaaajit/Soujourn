/* eslint-disable react/prop-types */
import { SlArrowDown } from "react-icons/sl";
function ChildNo({ dispatch, user }) {
  const { child, showChild } = user;
  return (
    <div className="w-[49%]">
      <div
        className="flex   justify-between text-lg px-3 py-3 border-[3px] border-[#dc8409] rounded-md"
        onClick={() => dispatch({ type: "showchild" })}
      >
        <p>Child</p>
        <p className="flex items-center gap-2">
          <span>{child}</span>
          <span>
            <SlArrowDown size="15px" />
          </span>
        </p>
      </div>
      {showChild && (
        <div className="flex justify-around py-3 bg-yellow-600/40 mt-2 rounded-md border-2 border-zinc-600/50">
          <button onClick={() => dispatch({ type: "subchild" })}>➖</button>
          <p className="font-semibold text-xl ">{child}</p>
          <button onClick={() => dispatch({ type: "addchild" })}>➕</button>
        </div>
      )}
    </div>
  );
}

export default ChildNo;
